...
import com.whl.wako.kernel.util.MaskUtils;
...
/*    ADDRESS( 0 , 0 , 0 ) {
        ...
        private final String mask = new String( MaskUtils.MASK_3 );
        @Override
        protected char[] internalMask(String str) {
            Matcher m = PATTERN.matcher( str );
            if(m.find()){
                return m.replaceAll( mask ).toCharArray();
            }
            return DEFAULT.mask(str).toCharArray();
        }
    },*/
    ...
    EMAIL( 0 , 0 , 0 ) {
        ...
        protected char[] internalMask(String str) {
            ...
            chs[0] = org[0];
            chs[1] = chs[2] = chs[3] = MaskUtils.MASK_CHAR;
    ...
    IP_ADDR( 0 , 0 , 0 ) {
        ...
        protected char[] internalMask(String str) {
            ...
            for ( int i = 0 , d = 0 ; i < chs.length ; i++ ) {
                ...
                if ( isMask ) {
                    chs[offset++] = MaskUtils.MASK_CHAR;
                    isMask = false;
                }
            }
    ...
    public final char[] maskToChars(String str) {
        ...
        if ( str.length() == 0 ) {
            return MaskUtils.EMPTY_CHARS;
        }
        return internalMask( str );
    }

    protected char[] internalMask(String str) {
        return MaskUtils.maskToChars( str, before, after, mask );
    }
}
